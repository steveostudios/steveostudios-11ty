...
// functions/updateBook.js
var import_airtable = __toESM(require_airtable());
var headers = {
  "Access-Control-Allow-Origin": "*",
  "Access-Control-Allow-Headers": "Content-Type",
  "Access-Control-Allow-Methods": "POST"
};
var ErrorWithStatus = (code, message) => {
  return {
    statusCode: code,
    headers,
...
exports.handler = async function(event, context) {
  var base = new import_airtable.default({ apiKey: process.env.AIRTABLE_API_KEY }).base(process.env.AIRTABLE_BASE_BOOKS_BASE);
  ...
  console.log(base);
  await base("Books").update(body.updates.map((book) => {
    const _a = book, { id } = _a, fields = __objRest(_a, ["id"]);
    return {
      id,
      fields
    };
