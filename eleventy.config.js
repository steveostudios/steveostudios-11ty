...
module.exports = function (eleventyConfig) {
  ...
  eleventyConfig.addLiquidFilter("debug", (content) => `<pre>${inspect(content)}</pre>`);

  // Shortcodes
  eleventyConfig.addLiquidShortcode("image", image);
  eleventyConfig.addLiquidShortcode("markdown", markdown);
  eleventyConfig.addLiquidShortcode("booksByYearSVG", (books) => lineGraph(books, "books"));
  eleventyConfig.addLiquidShortcode("pagesByYearSVG", (books) =>lineGraph(books, "pages", { gap: 1000 }));
